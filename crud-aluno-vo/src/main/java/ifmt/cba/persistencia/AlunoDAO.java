...
public class AlunoDAO extends DAO {
    ...
    public AlunoDAO(ConexaoBD conexao) throws PersistenciaException {
        ...
        try {
            ...
            comandoAlterar = conexao.getConexao().prepareStatement("UPDATE Aluno SET nome=?,nomemae=?,nomepai=?,sexo=?,"
...
comandoIncluir.setInt(4,alunoVO.getSexo().ordinal());//salvaovalorordinalda enumeração;
...
comandoIncluir.setString(9,alunoVO.getEndereco().getUf().name());//salvaovalor String da enumeração
...
throw new PersistenciaException("Erroaoexcluiroaluno−"+ex.getMessage());
}
return retorno;
}
...
String comandoSQL="SELECT * FROM Aluno WHERE UPPER(nome)LIKE'"+nome.trim().toUpperCase
...
throw new PersistenciaException("Erronaselecaoopornome−"+ex.getMessage());
}
return listaAluno;
}
    ...
    private AlunoVO montaAlunoVO(ResultSet rs) throws PersistenciaException {
        ...
        if (rs != null) {
            try {
                ...
                throw new PersistenciaException("Erroaoacessarosdadosdoresultado");
            }
        }
        return alu;
    }
